...
// ── All date logic runs ONLY on client, never on server ───────────────────────
...
export function TicketBoothRevenueSummary({ boatId, vesselName }: Props) {
  // ── mounted guard — nothing date-related renders on server ────────────────
  ...
  // ── Don't render anything until client-side — no hydration mismatch ───────
  if (!mounted) {
    return (
              ...
              <table className="min-w-full text-sm">
                ...
                <tfoot>
                  <tr className="bg-teal-50/60 border-t-2 border-teal-200">
                    <td colSpan={4} className="px-4 py-2.5 text-xs font-bold text-[#134e4a]">{label} Total</td>
