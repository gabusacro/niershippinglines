...
function buildHtml(params: SendBookingConfirmedParams, branding: SiteBranding): string {
...
<head>
  ...
  <title>Payment confirmed – ${branding.site_name}</title>
</head>
...
export async function sendBookingConfirmed(params: SendBookingConfirmedParams): Promise<boolean> {
  ...
  const subject = `Payment confirmed – Reference ${params.reference} – ${branding.site_name}`;
