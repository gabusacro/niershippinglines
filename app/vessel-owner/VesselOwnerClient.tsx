...
export function VesselOwnerClient({
  ...
  // Allow going one month ahead (to see future bookings)
  ...
  return (
    <div className="mx-auto max-w-5xl px-4 py-8 sm:px-6 lg:px-8 space-y-6">
      ...
      <div className="rounded-2xl bg-gradient-to-br from-[#0c7b93] to-[#134e4a] px-6 py-7 text-white shadow-lg">
        ...
        <div className="mt-4 flex items-center gap-3 flex-wrap">
          ...
          <span className="text-base font-bold">
            ...
            {isViewingNextMonth && <span className="ml-2 rounded-full bg-white/20 px-2 py-0.5 text-xs font-medium">Next Month</span>}
          </span>
          <button
            onClick={nextMonth}
            disabled={isAtMax}
            className="rounded-lg bg-white/15 px-3 py-1.5 text-sm font-semibold hover:bg-white/25 transition-colors disabled:opacity-30 disabled:cursor-not-allowed">
            Next →
          </button>
          {isViewingNextMonth && (
      ...
      {/* ── Next Month Early Booking Alert (last 5 days only, viewing current month) ── */}
      {nextMonthPreview && !isViewingNextMonth && (
        <div className="rounded-xl border-2 border-blue-300 bg-gradient-to-br from-blue-50 to-indigo-50 p-5 shadow-sm">
          <div className="flex items-start justify-between gap-4 flex-wrap">
            <div>
              ...
              <p className="mt-1 text-xs text-blue-700">
                Passengers have already booked for next month. Here&apos;s your early headcount.
              </p>
            </div>
            <button
          ...
          {nextMonthPreview.tripCount === 0 ? (
            <p className="mt-4 text-sm text-blue-600">No trips scheduled yet for {nextMonthPreview.monthName}.</p>
          ) : (
            <div className="mt-4 grid grid-cols-2 sm:grid-cols-4 gap-3">
              <div className="rounded-lg bg-white/70 p-3 text-center">
                <p className="text-xs text-blue-600 font-medium">Scheduled Trips</p>
                <p className="mt-0.5 text-xl font-bold text-blue-900">{nextMonthPreview.tripCount}</p>
              </div>
              <div className="rounded-lg bg-white/70 p-3 text-center">
                <p className="text-xs text-[#0c7b93] font-medium">🌐 Online Pax</p>
                <p className="mt-0.5 text-xl font-bold text-[#0c7b93]">{nextMonthPreview.onlinePax}</p>
              </div>
              <div className="rounded-lg bg-white/70 p-3 text-center">
                <p className="text-xs text-amber-700 font-medium">🚶 Walk-in Pax</p>
                <p className="mt-0.5 text-xl font-bold text-amber-800">{nextMonthPreview.walkInPax}</p>
              </div>
              <div className="rounded-lg bg-white/70 p-3 text-center">
                <p className="text-xs text-[#0f766e] font-medium">Est. Online Fare</p>
                <p className="mt-0.5 text-xl font-bold text-[#134e4a]">{peso(nextMonthPreview.onlineNetFareCents)}</p>
              </div>
            </div>
          )}

          <p className="mt-3 text-xs text-blue-500">
            This preview disappears on {nextMonthPreview.monthName} 1 when it becomes the active month. Click &quot;View {nextMonthPreview.monthName}&quot; for full trip details.
          </p>
        </div>
      )}
      ...
      {/* Operator Loyalty Bonus — only show for current/past months */}
      {!isViewingNextMonth && (
      ...
      <div>
        <div className="flex items-center justify-between flex-wrap gap-2">
          <h2 className="text-sm font-bold uppercase tracking-wide text-[#0f766e]">
            {isViewingNextMonth ? `Upcoming Trips — ${MONTH_NAMES[selectedMonth - 1]} ${selectedYear}` : `All Trips — ${MONTH_NAMES[selectedMonth - 1]} ${selectedYear}`}
                ...
                <tr className="bg-slate-50">
                  <td colSpan={9} className="px-4 py-4">
                    ...
                    <p className="mt-2 text-xs text-slate-400">
                      🌐 Online fare = total paid minus platform fees (remitted by Travela Siargao) · 🚶 Walk-in cash = collected directly by vessel
                    </p>
                  </td>
                </tr>
              )}
