...
const ID_LABELS: Record<string, string> = {
  ...
  philsys: "PhilSys / National ID",   prc: "PRC ID", voter_id: "Voter's ID",
  other: "Government ID",
};
...
export default async function ParkingBookingDetailPage({
  params,
}: {
  ...
  const { data: booking } = await supabase
    ...
    .select(`
      ...
      total_days, vehicle_count, vehicles,
      parking_fee_cents, platform_fee_cents, processing_fee_cents, total_amount_cents,
      lot_snapshot_name, lot_snapshot_address, lot_snapshot_distance, rate_snapshot_label,
      overstay_days, overstay_fee_cents, overstay_payment_status,
      admin_notes, checked_in_at, checked_out_at, created_at
    `)
    .eq("id", id)
      ...
      <div style={{ background: "linear-gradient(135deg,#064e3b 0%,#0c7b93 100%)" }}>
        <div className="mx-auto max-w-2xl px-4 py-10 sm:px-6">
          ...
          <div className="flex items-start justify-between gap-3 mt-1">
            ...
            <span className={`shrink-0 rounded-full border px-3 py-1 text-xs font-bold ${meta.color}`}>
              {meta.icon} {meta.label}
            </span>
          </div>
          <div className="mt-4">
            <Link href="/dashboard/parking" className="text-xs text-white/60 hover:text-white/90 transition-colors">
              ← My Parking Bookings
            </Link>
          </div>
        </div>
      ...
      <div className="mx-auto max-w-2xl px-4 py-8 sm:px-6 space-y-5">
        ...
        {booking.status === "pending_payment" && (
          <div className="rounded-2xl border-2 border-amber-200 bg-amber-50 p-5">
            ...
            <p className="text-sm text-amber-700">
              Your documents and GCash payment have been submitted. Admin will verify and confirm your slot.
              Your slot is <strong>not yet locked</strong> — you&apos;ll be notified once approved.
            </p>
          </div>
        )}
        ...
        {booking.status === "overstay" && (
          <div className="rounded-2xl border-2 border-red-200 bg-red-50 p-5">
            ...
            <p className="text-sm text-red-700">
              Your parking period has expired. Overstay: <strong>{booking.overstay_days} day{booking.overstay_days > 1 ? "s" : ""}</strong> —
              additional fee: <strong>{peso(booking.overstay_fee_cents)}</strong>. Please settle at the lot before exit.
            </p>
          </div>
        )}
        ...
        <div className="rounded-2xl border-2 border-teal-200 bg-white overflow-hidden">
          ...
          <div className="px-5 py-4 space-y-2 text-sm">
            <div className="flex justify-between">
              <span className="text-gray-600">Parking fee ({booking.vehicle_count} vehicle{booking.vehicle_count > 1 ? "s" : ""} × {booking.total_days} day{booking.total_days > 1 ? "s" : ""})</span>
              <span className="font-semibold">{peso(booking.parking_fee_cents)}</span>
            </div>
            <div className="flex justify-between text-xs text-gray-400">
              <span>Platform Service Fee</span><span>{peso(booking.platform_fee_cents)}</span>
            </div>
            <div className="flex justify-between text-xs text-gray-400">
              <span>Payment Processing Fee</span><span>{peso(booking.processing_fee_cents)}</span>
            </div>
            {booking.overstay_fee_cents > 0 && (
              <div className="flex justify-between text-xs text-red-600">
                <span>Overstay Fee ({booking.overstay_days} day{booking.overstay_days > 1 ? "s" : ""})</span>
                <span>{peso(booking.overstay_fee_cents)}</span>
              </div>
            )}
            <div className="flex justify-between pt-2 border-t-2 border-teal-200">
              <span className="font-black text-[#134e4a]">Total Paid</span>
              <span className="font-black text-xl text-[#0c7b93]">{peso(booking.total_amount_cents)}</span>
            </div>
          </div>
          <div className="px-5 pb-4">
            {booking.payment_status === "paid" ? (
              <div className="rounded-xl bg-emerald-50 border border-emerald-200 px-4 py-2.5 text-sm text-emerald-800 font-semibold">
                ✅ Payment confirmed by admin
              </div>
            ) : booking.payment_proof_path ? (
              <div className="rounded-xl bg-blue-50 border border-blue-200 px-4 py-2.5 text-sm text-blue-800">
                📎 GCash screenshot submitted — awaiting admin verification
                {booking.gcash_transaction_reference && (
                  <p className="text-xs text-blue-600 mt-1">Ref: {booking.gcash_transaction_reference}</p>
                )}
              </div>
            ) : (
              <div className="rounded-xl bg-amber-50 border border-amber-200 px-4 py-2.5 text-sm text-amber-800">
                ⏳ No payment proof on record
              </div>
            )}
          </div>
        </div>
        ...
        <div className="flex flex-wrap gap-3 pb-6">
          <Link href="/dashboard/parking"
            className="rounded-xl border-2 border-teal-200 bg-white px-5 py-2.5 text-sm font-semibold text-[#134e4a] hover:bg-teal-50 transition-colors">
            ← All Bookings
          </Link>
          <Link href="/parking"
            className="rounded-xl border-2 border-teal-200 bg-white px-5 py-2.5 text-sm font-semibold text-[#134e4a] hover:bg-teal-50 transition-colors">
