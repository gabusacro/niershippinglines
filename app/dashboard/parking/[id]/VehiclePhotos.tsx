...
const LABEL_META: Record<string, { label: string; color: string; emoji: string }> = {
  arrival:   { label: "Arrival",   color: "bg-blue-50 text-blue-800 border-blue-200",      emoji: "🟦" },
  departure: { label: "Departure", color: "bg-emerald-50 text-emerald-800 border-emerald-200", emoji: "🟩" },
  damage:    { label: "Damage",    color: "bg-red-50 text-red-800 border-red-200",          emoji: "🔴" },
...
export default function VehiclePhotos({ reservationId }: { reservationId: string }) {
  const [photos, setPhotos] = useState<Photo[]>([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    fetch(`/api/parking/photos?reservation_id=${reservationId}`)
      .then(r => r.ok ? r.json() : [])
  ...
  if (photos.length === 0) return (
    <div className="rounded-2xl border-2 border-teal-100 bg-white p-5">
      <h2 className="text-sm font-black text-[#134e4a] mb-2">📸 Vehicle Condition Photos</h2>
      <p className="text-sm text-gray-400">No condition photos yet. Crew will upload photos when your vehicle arrives and departs.</p>
    </div>
  );
  ...
  return (
    <div className="rounded-2xl border-2 border-teal-100 bg-white overflow-hidden">
      <div className="bg-teal-50 px-5 py-3 border-b border-teal-100">
        ...
        <p className="text-xs text-gray-400 mt-0.5">Uploaded by parking crew. Photos expire after 6 months.</p>
      </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
              {platePhotos.map(photo => {
                ...
                const uploaderName = photo.uploader
                  ? (Array.isArray(photo.uploader)
                    ? (photo.uploader as { full_name?: string }[])[0]?.full_name
                    : (photo.uploader as { full_name?: string }).full_name)
                  : null;
                return (
                  <div key={photo.id} className="rounded-xl border border-teal-100 overflow-hidden bg-white">
                    {photo.signed_url ? (
                      <a href={photo.signed_url} target="_blank" rel="noopener noreferrer">
                        <img src={photo.signed_url} alt={`${meta.label} — ${plate}`}
                    ...
                    <div className="p-2 space-y-1">
                      ...
                      <p className="text-xs text-gray-400">{formatDateTime(photo.uploaded_at)}</p>
