...
import {
  Image as ImageIcon, Sparkles, Search, Plus,
...
// ── AI Guidance fields ────────────────────────────────────────────────────────
function AiGuidanceFields({ form, set }: {
  form: AttractionForm;
  set: <K extends keyof AttractionForm>(k: K, v: AttractionForm[K]) => void;
}) {
  ...
  return (
    <div className="rounded-2xl border border-slate-100 bg-white p-5 space-y-4">
      <div className="flex items-center gap-2">
        ...
        <h2 className="text-[14px] font-semibold text-slate-700">AI guidance</h2>
        <span style={{ fontSize: 9, fontWeight: 700, padding: "2px 7px", borderRadius: 999, background: "#F3E8FF", color: "#7C3AED", border: "1px solid #DDD6FE" }}>
          Controls what AI writes
        </span>
      </div>
      <p className="text-[11px] text-slate-400 -mt-2">
        Fill in what you know. Leave blank what you don't. AI uses only what you provide — no guessing.
      </p>
      <div className="grid grid-cols-2 gap-3">
        <div>
          <label className={lbl}>Barangay / area</label>
          <input className={input} value={form.location_barangay ?? ""} onChange={(e) => set("location_barangay" as any, e.target.value)} placeholder="e.g. Barangay Sohoton" />
        </div>
        <div>
          <label className={lbl}>Municipality</label>
          <input className={input} value={form.municipality ?? ""} onChange={(e) => set("municipality" as any, e.target.value)} placeholder="e.g. Socorro" />
        </div>
      </div>
      <div className="grid grid-cols-2 gap-3">
        <div>
          <label className={lbl}>Entrance / env fee</label>
          <input className={input} value={form.entrance_fee ?? ""} onChange={(e) => set("entrance_fee" as any, e.target.value)} placeholder="e.g. ₱150 per person" />
        </div>
        <div>
          <label className={lbl}>Travel time from General Luna</label>
          <input className={input} value={form.travel_time ?? ""} onChange={(e) => set("travel_time" as any, e.target.value)} placeholder="e.g. 1.5 to 2 hours by boat" />
        </div>
      </div>
      <div>
        <label className={lbl}>Best time to visit</label>
        <input className={input} value={form.best_time ?? ""} onChange={(e) => set("best_time" as any, e.target.value)} placeholder="e.g. Early morning, low tide, dry season March–October" />
      </div>
      <div>
        <label className={lbl}>What visitors actually do there</label>
        <textarea className={input + " resize-y"} rows={3} value={form.what_to_expect ?? ""}
          onChange={(e) => set("what_to_expect" as any, e.target.value)}
          placeholder="e.g. Swim in jellyfish lagoon, ride a boat through caves, cliff jump, eat boodle fight lunch after" />
      </div>
      <div>
        <label className={lbl}>Must mention in content</label>
        <input className={input} value={form.must_mention ?? ""} onChange={(e) => set("must_mention" as any, e.target.value)} placeholder="e.g. stingless jellyfish, cave entrance, boat ride from Socorro" />
      </div>
      <div>
        <label className={lbl}>Do NOT mention / avoid saying</label>
        <input className={input} value={form.avoid_mention ?? ""} onChange={(e) => set("avoid_mention" as any, e.target.value)} placeholder="e.g. do not say luxury, do not exaggerate the food, avoid cinematic language" />
      </div>
      <div>
        <label className={lbl}>Practical tips (what to bring, rules, warnings)</label>
        <textarea className={input + " resize-y"} rows={3} value={form.tips ?? ""}
          onChange={(e) => set("tips" as any, e.target.value)}
          placeholder="e.g. Bring water shoes, wear rash guard, no plastic allowed, book through a tour operator" />
      </div>
      <div>
        <label className={lbl}>AI writing tone</label>
        <div className="grid grid-cols-3 gap-2">
          {[
            { key: "grounded", label: "Grounded", desc: "Practical, honest, local" },
            { key: "warm", label: "Warm", desc: "Friendly, inviting, clear" },
            { key: "editorial", label: "Editorial", desc: "Magazine style, polished" },
          ].map((t) => (
            <button key={t.key} type="button" onClick={() => set("ai_tone" as any, t.key)}
              className={`rounded-xl border p-2.5 text-left transition-all ${(form as any).ai_tone === t.key ? "border-purple-400 bg-purple-50" : "border-slate-200 bg-white hover:border-purple-300"}`}>
              <div className={`text-[12px] font-bold ${(form as any).ai_tone === t.key ? "text-purple-700" : "text-slate-700"}`}>{t.label}</div>
              <div className={`text-[10px] mt-0.5 ${(form as any).ai_tone === t.key ? "text-purple-500" : "text-slate-400"}`}>{t.desc}</div>
            </button>
          ))}
        </div>
      </div>
...
function GenerateFullContentButton({ title, description, category, autoLinks, layoutStyle,
  municipality, locationBarangay, entranceFee, travelTime, bestTime,
  whatToExpect, mustMention, avoidMention, tips, aiTone, onDone }: {
  title: string; description: string; category: string;
  autoLinks: AutoLink[]; layoutStyle: LayoutStyle;
  municipality: string; locationBarangay: string; entranceFee: string;
  travelTime: string; bestTime: string; whatToExpect: string;
  mustMention: string; avoidMention: string; tips: string; aiTone: string;
  ...
  async function generate() {
    ...
    try {
      const res = await fetch("/api/admin/attractions/generate-seo", {
        ...
        body: JSON.stringify({
          title, description, category, mode: "full",
          autoLinks, layoutStyle,
          municipality, locationBarangay, entranceFee,
          travelTime, bestTime, whatToExpect,
          mustMention, avoidMention, tips, aiTone,
        }),
      });
  ...
  return (
    <button type="button" onClick={generate} disabled={state === "loading"}
      className={`inline-flex items-center gap-2 px-3 py-1.5 rounded-lg text-[12px] font-semibold transition-all border ${state === "done" ? "bg-green-50 text-green-700 border-green-200" :
  ...
  return (
    <div className="flex flex-col items-end gap-1">
      <button type="button" onClick={generate} disabled={state === "loading"}
        ...
        {state === "loading" ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : state === "done" ? <Check className="w-3.5 h-3.5" /> : state === "error" ? <span>x</span> : <Sparkles className="w-3.5 h-3.5" />}
  ...
  return (
    <div className="flex flex-col items-end gap-1">
      <button type="button" onClick={enhance} disabled={state === "loading"}
        ...
        {state === "loading" ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : state === "done" ? <Check className="w-3.5 h-3.5" /> : state === "error" ? <span>x</span> : <Wand2 className="w-3.5 h-3.5" />}
...
function AttractionFormPanel({ item, onSave, onDelete, onCancel }: {
  ...
  const [form, setForm] = useState<AttractionForm>(
    item ? {
      ...
      location_barangay: (item as any).location_barangay ?? "",
      municipality: (item as any).municipality ?? "",
      entrance_fee: (item as any).entrance_fee ?? "",
      travel_time: (item as any).travel_time ?? "",
      best_time: (item as any).best_time ?? "",
      what_to_expect: (item as any).what_to_expect ?? "",
      must_mention: (item as any).must_mention ?? "",
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-6 space-y-4">
      ...
      <div className={card}>
        ...
        <div>
          <div className="flex items-center justify-between mb-2">
            ...
            <div className="flex items-center gap-2 flex-wrap justify-end">
              ...
              <GenerateFullContentButton
                ...
                municipality={(form as any).municipality ?? ""}
                locationBarangay={(form as any).location_barangay ?? ""}
                entranceFee={(form as any).entrance_fee ?? ""}
                travelTime={(form as any).travel_time ?? ""}
                bestTime={(form as any).best_time ?? ""}
                whatToExpect={(form as any).what_to_expect ?? ""}
                mustMention={(form as any).must_mention ?? ""}
          ...
          <p className="text-[10px] text-slate-400 mt-1">
            Type notes then click Generate full content for 800+ words with H2 sections. Rewrite polishes existing text.
          </p>
        </div>
      ...
      {/* AI Guidance card */}
      <AiGuidanceFields form={form} set={set} />
      ...
      <div className={card}>
        ...
        <p className="text-[11px] text-slate-400 -mt-2">
          Set phrases that get automatically linked when content is generated. Min 2, max 8.
        </p>
...
export function AttractionsAdminPage({ initialItems, initialAds = [] }: { initialItems: Attraction[]; initialAds?: any[] }) {
  ...
  async function handleSave(form: AttractionForm & { hero_position: string; photos: Photo[]; meta_description: string; description_html: string }) {
    const body = {
      ...
      location_barangay: (form as any).location_barangay ?? null,
      municipality: (form as any).municipality ?? null,
      entrance_fee: (form as any).entrance_fee ?? null,
      travel_time: (form as any).travel_time ?? null,
      best_time: (form as any).best_time ?? null,
      what_to_expect: (form as any).what_to_expect ?? null,
      must_mention: (form as any).must_mention ?? null,
      avoid_mention: (form as any).avoid_mention ?? null,
      tips: (form as any).tips ?? null,
      ai_tone: (form as any).ai_tone ?? "grounded",
    };
