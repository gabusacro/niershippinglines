...
export default async function AdminBookingDetailPage({
  ...
  const rescheduleHistoryWithUrls = await Promise.all(
    (rescheduleHistory ?? []).map(async (r) => ({
      ...r,
 ...
 {rescheduleHistoryWithUrls.length > 0 && (
          <div className="mt-4 rounded-lg border border-orange-200 bg-orange-50/50 p-4">
            <h3 className="text-sm font-semibold text-[#134e4a]">Reschedule History ({rescheduleHistoryWithUrls.length})</h3>
            <ul className="mt-2 space-y-3">
              {rescheduleHistoryWithUrls.map((r) => (
                <li key={r.id} className="rounded-lg bg-white px-3 py-3 text-xs border border-orange-100">
                  ...
                  {r.proofUrl && (
                    <div className="mt-2">
                      <p className="text-xs font-semibold text-teal-700 mb-1">Payment screenshot</p>
                      <PaymentProofViewer
                        proofUrl={r.proofUrl}
                        isPdf={r.proof_path?.toLowerCase().endsWith(".pdf")}
                        thumbnailClassName="h-24 w-auto max-w-[160px] rounded object-contain"
                      />
                      <p className="mt-0.5 text-xs text-teal-600">Click to view full size.</p>
                    </div>
                  )}
                </li>
              ))}
