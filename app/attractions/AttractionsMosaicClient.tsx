...
const CATEGORY_EMOJI: Record<string, string> = {
  Surfing: "🏄", Islands: "🏝️", Beaches: "🌅", Adventure: "🧗", "Rivers & Caves": "🌿",
};
...
function randomInterval(): number { return Math.floor(Math.random() * 15_000) + 30_000; }
function resolveImageUrls(a: AttractionItem): string[] {
  if (a.image_urls?.length) return a.image_urls;
...
// ─── Card ─────────────────────────────────────────────────────────────────────
function MosaicCard({
  attraction, baseHeight, isExpanded, isAnyExpanded, onToggle, onViewPhotos,
}: {
  attraction: AttractionItem; baseHeight: number;
  isExpanded: boolean; isAnyExpanded: boolean;
  onToggle: () => void; onViewPhotos: () => void;
}) {
  const imageUrls = resolveImageUrls(attraction);
  const hasImages = imageUrls.length > 0;
  const emoji = getEmoji(attraction.category);
  const isTall = baseHeight >= 300;
  const finalH = isExpanded ? 580 : baseHeight;
  ...
  return (
    <div
      ...
      style={{
        ...
        transition: [
          ...
          "box-shadow 0.28s ease", "border-radius 0.35s ease",
        ].join(", "),
        ...
        boxShadow: isExpanded
          ...
          : isAnyExpanded ? "0 2px 6px rgba(0,0,0,0.06)" : "0 3px 12px rgba(0,0,0,0.09)",
    ...
    >
      ...
      <div style={{
        ...
        transform: isExpanded ? "scale(1.04)" : "scale(1)", zIndex: 0,
      }}>
        {!hasImages && (
          <div style={{
      ...
      <div style={{
        ...
        <h2 style={{
          ...
          textShadow: "0 2px 12px rgba(0,0,0,0.5)", letterSpacing: "-0.02em",
        }}>
          {attraction.title}
        </h2>

        {!isExpanded && isTall && attraction.short && (
          <p style={{ fontSize: 12, color: "rgba(255,255,255,0.7)", lineHeight: 1.5, marginTop: 4, marginBottom: 0 }}>
            {attraction.short}
          </p>
        )}
        ...
        <div style={{
          ...
          {attraction.short && (
            <p style={{ fontSize: 14, color: "rgba(255,255,255,0.75)", lineHeight: 1.5, marginBottom: 8, fontStyle: "italic" }}>
              {attraction.short}
            </p>
          )}
          ...
          <div style={{ display: "flex", flexWrap: "wrap", gap: 7, marginBottom: 18 }}>
            {attraction.category && (
              <span style={{
                ...
                background: "rgba(12,123,147,0.55)", border: "1px solid rgba(12,123,147,0.8)",
              }}>{emoji} {attraction.category}</span>
            )}
            {hasImages && (
              <button type="button" onClick={(e) => { e.stopPropagation(); onViewPhotos(); }}
                style={{
                  padding: "4px 12px", borderRadius: 999, fontSize: 12, fontWeight: 700, color: "#fff",
...
export function AttractionsMosaicClient({ attractions }: { attractions: AttractionItem[] }) {
  ...
  const scheduleNext = useCallback(() => {
    ...
    timerRef.current = setTimeout(() => {
      ...
      setFading(true);
      setTimeout(() => { setSeed((s) => s + 1); setExpanded(null); setFading(false); scheduleNext(); }, 320);
    }, randomInterval());
  }, []);
  ...
  const handleToggle = useCallback((id: string | number) => {
    ...
    if (timerRef.current) clearTimeout(timerRef.current);
    timerRef.current = setTimeout(() => { pausedRef.current = false; scheduleNext(); }, 8000);
  }, [scheduleNext]);
  ...
  const handleModalClose = useCallback(() => {
    setModal(null);
    timerRef.current = setTimeout(() => { pausedRef.current = false; scheduleNext(); }, 5000);
  }, [scheduleNext]);
  ...
  // Shortest-column-first masonry — flex columns, zero gap issues
  ...
  visible.forEach((a, i) => {
    ...
    // If this card is expanded, it needs 3 cols — place in col 0 and mark
  ...
  return (
    <>
      {modal && (
        <AttractionImageModal title={modal.title} imageUrls={modal.imageUrls}
          initialIndex={modal.initialIndex} onClose={handleModalClose} />
      )}
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-8">
            ...
            <div className="flex flex-wrap gap-2">
              {CATEGORIES.map((cat) => (
                <button key={cat} onClick={() => setActiveCategory(cat)}
                  className={`px-3 py-1.5 rounded-full text-xs font-bold tracking-wide transition-all border ${activeCategory === cat
        ...
        {/* Masonry — flex columns, no CSS grid row-span issues */}
        {visible.length === 0 ? (
          <div className="text-center py-20">
            <div className="text-5xl mb-3">🔍</div>
            <p className="text-[#0f766e] font-semibold">No attractions found</p>
            <button onClick={() => { setSearch(""); setActiveCategory("All"); }}
              className="mt-4 px-4 py-2 rounded-xl border border-teal-200 text-sm text-[#0f766e] font-semibold hover:border-[#0c7b93] transition-all bg-white">
              Clear filters
            </button>
          </div>
        ) : (
          <div style={{
            ...
            {cols.map((colItems, ci) => (
              <div key={`col-${ci}-${seed}`} style={{ flex: 1, minWidth: 0, display: "flex", flexDirection: "column", gap: 12 }}>
