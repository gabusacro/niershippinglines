...
// Static Siargao attractions (will be admin-editable from Supabase later)
const ATTRACTIONS = [
  {
    slug: "cloud-9",
    title: "Cloud 9",
  ...
  },
  {
    ...
    description: "A peaceful turquoise lagoon surrounded by mangroves. Rent a kayak or paddleboard, swim, or relax on the floating deck. Often combined with island-hopping tours. A refreshing escape from the open sea.",
    icon: "palm",
  },
  {
    slug: "naked-island",
  ...
  },
  {
    slug: "taktak-falls",
    title: "Taktak Falls",
    short: "Waterfall and natural pool in the interior.",
    description: "A short ride from the coast, Taktak Falls offers a freshwater pool and a scenic cascade. Good for a half-day trip when you want a break from the beach. The area is green and quiet.",
    icon: "palm",
  },
  {
    slug: "general-luna",
    title: "General Luna & Beach Road",
    short: "Surf town with cafes, shops, and beach vibes.",
    description: "General Luna is the main tourist hub: surf schools, cafes, restaurants, and beachfront bars. The beach road runs along the coast—rent a motorbike or walk to feel the island pace. Nightlife and day trips start here.",
    icon: "surf",
  },
];
    ...
    <div className="mx-auto max-w-4xl px-4 py-8 sm:px-6 sm:py-12 lg:px-8">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:gap-3 mb-8 sm:mb-10">
        ...
        <div className="min-w-0">
          ...
          <p className="text-sm text-[#0f766e] sm:text-base">What to see and do on the island. Content is static for now—will be editable by admin later.</p>
        </div>
      </div>
      ...
      <div className="grid gap-6 sm:gap-8">
        {ATTRACTIONS.map((a) => {
          ...
          return (
            <article
              key={a.slug}
              className="rounded-2xl border border-teal-200 bg-white/80 shadow-sm overflow-hidden hover:shadow-md transition-shadow"
            >
              <div className="flex flex-col gap-4 sm:flex-row sm:gap-4 p-4 sm:p-6 md:p-8">
                ...
                <div className="min-w-0">
                  ...
                  <Link
                    href={ROUTES.book}
                    className="mt-4 inline-flex min-h-[44px] items-center gap-1 text-sm font-semibold text-[#0c7b93] hover:underline touch-target py-2"
                  >
      ...
      <div className="mt-10 sm:mt-12 rounded-xl bg-[#fef3c7]/50 border border-teal-100 p-4 sm:p-6 text-center">
        ...
        <Link
          href={ROUTES.book}
          className="mt-4 inline-flex min-h-[48px] items-center justify-center rounded-xl bg-[#0c7b93] px-6 py-3 text-sm font-semibold text-white hover:bg-[#0f766e] transition-colors touch-target w-full sm:w-auto"
        >
          Book a trip
        </Link>
      </div>
