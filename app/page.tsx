...
export const metadata: Metadata = {
  ...
  openGraph: {
    ...
    images: [{ url: "https://gohrllugnblfzsypapee.supabase.co/storage/v1/object/public/promo-media/promo-popup/banner-1773400190201.png", width: 1200, height: 630, alt: "Travela Siargao – Ferry Booking to Siargao Island" }],
  },
  twitter: {
    card: "summary_large_image", title: "Travela Siargao | Ferry Booking",
  ...
  return (
    <div className="min-h-[calc(100vh-8rem)] overflow-x-hidden">
      ...
      {/* ═══════════════════════════════════════════════════════════════
          ...
          — Hero image on top, BG2 fixed below via layered backgrounds
         ═══════════════════════════════════════════════════════════════ */}
      <div className="relative">

        {/* ① HERO SECTION — own background image */}
        <section className="relative min-h-screen flex flex-col justify-end overflow-hidden">
          ...
          {/* Hero overlay — darkens heavily at the bottom so it blends into BG2 */}
          <div className="absolute inset-0"
            style={{
              background: "linear-gradient(to bottom, rgba(13,74,69,0.35) 0%, rgba(8,92,82,0.15) 40%, rgba(5,20,15,0.85) 80%, rgba(3,12,9,0.98) 100%)"
            }}
          />

          {/* Floating particles */}
          <div className="absolute inset-0 overflow-hidden pointer-events-none z-[2]" aria-hidden="true">
            {[{w:5,l:12,d:9,delay:0},{w:4,l:78,d:12,delay:2.5},{w:7,l:45,d:8,delay:1},{w:3,l:62,d:13,delay:4},{w:6,l:28,d:10,delay:3},{w:4,l:88,d:11,delay:6},{w:5,l:55,d:7,delay:2},{w:3,l:35,d:14,delay:5}].map((p,i) => (
          ...
          {/* Bottom of hero — no wave, just fades to black so BG2 appears seamlessly */}
          <div className="relative z-10 h-24 w-full" />
        </section>

        {/* ═══════════════════════════════════════════════════════════════
            BG2 SECTION — fixed background, starts right after hero
           ═══════════════════════════════════════════════════════════════ */}
        <div
          style={{
        ...
        >
          ...
          <div className="relative" style={{ background: "linear-gradient(to bottom, rgba(3,12,9,0.92) 0%, rgba(5,25,18,0.82) 30%, rgba(7,30,22,0.78) 100%)" }}>

            {/* ② SCHEDULE */}
            <div className="relative z-10 mx-auto max-w-5xl px-4 sm:px-6 pt-10 pb-8" id="schedule">
            ...
            <section className="relative z-10 py-12 sm:py-16">
              <div className="mx-auto max-w-5xl px-4 sm:px-6">
                ...
                <h2 className="mt-1 text-[clamp(1.5rem,3vw,2.1rem)] font-black text-white">Book in 3 easy steps</h2>
                ...
                <div className="mt-8 grid sm:grid-cols-3 gap-4">
                  {[
                    ...
                    <div key={i} className="relative rounded-2xl border border-white/10 text-center px-6 py-6"
                      ...
                      <p className="mt-1 text-sm font-bold text-white/55">{step.desc}</p>
                      {i < 2 && <span className="absolute -right-3 top-1/2 -translate-y-1/2 text-[#4dd9c0] hidden sm:block z-10"><ArrowRight size={22}/></span>}
                    </div>
                  ))}
                </div>

                <div className="mt-6 flex gap-3 rounded-2xl border border-amber-400/25 px-5 py-4"
                  ...
                  <div>
                    ...
                    <p className="mt-1 text-xs font-bold text-amber-200/65 leading-relaxed">
                ...
                <div className="mt-4 flex flex-wrap items-center justify-between gap-4 rounded-2xl border border-[#f59e0b]/25 px-5 py-4"
                  style={{ background: "linear-gradient(to right, rgba(245,158,11,0.18), rgba(251,191,36,0.10))", backdropFilter: "blur(8px)" }}>
                  <div>
                    <h3 className="font-black text-white text-lg">Discounts for all ages</h3>
                    <p className="text-sm font-bold text-white/55 mt-0.5">Senior, PWD, student & child fares — bring valid ID</p>
                    <div className="mt-2 flex flex-wrap gap-2">
                      {[`👴 Senior -${fees.senior_discount_percent}%`,`♿ PWD -${fees.pwd_discount_percent}%`,"👶 Infants free",`🎒 Kids -${fees.child_discount_percent}%`].map(t => (
                        <span key={t} className="rounded-full bg-white/10 px-3 py-0.5 text-xs font-extrabold text-white">{t}</span>
                      ))}
                    </div>
                  </div>
                  <Link href={ROUTES.book} className="rounded-xl bg-[#0c7b93] px-5 py-2.5 text-sm font-extrabold text-white hover:bg-[#0f766e] whitespace-nowrap transition-colors">
                    Book now →
                  </Link>
                </div>
              </div>
              ...
              <div className="mx-auto max-w-5xl px-4 sm:px-6">
                <div className="grid lg:grid-cols-2 gap-12 items-center">
                  <div>
                    ...
                    <p className="mt-4 text-white/65 font-semibold leading-relaxed text-sm sm:text-base">
