...
type Settings = {
  carRate: number; motorcycleRate: number; vanRate: number | null;
  platformFee: number; processingFee: number; commission: number;
  maxDays: number; requiredDocs: string;
...
type VehicleForm = {
  vehicle_type: string; plate_number: string; make_model: string; color: string;
  or_cr_number: string; driver_id_type: string; driver_id_number: string;
  or_cr_file: File | null; id_photo_file: File | null;
};
...
// Max vehicles = total confirmed-available slots, capped at 10, default 3
function getMaxVehicles(lot: ParkingLot): number {
  const total = lot.available_car + lot.available_motorcycle + lot.available_van;
  return total > 0 ? Math.min(total, 10) : 3;
}
...
// Alternating card backgrounds — odd=teal wash, even=gray wash
const CARD_BG = [
  "bg-teal-50 border-teal-200",
  "bg-gray-50 border-gray-200",
];

// Photo compression to WebP with progressive quality reduction
    ...
    img.onload = () => {
      ...
      const tryQuality = (q: number) => {
        canvas.toBlob((blob) => {
          ...
          if (blob.size > 3 * 1024 * 1024 && q > 0.55) { tryQuality(q - 0.15); return; }
...
function FileUploadButton({ label, file, onChange }: { label: string; file: File | null; onChange: (f: File | null) => void }) {
  const ref = useRef<HTMLInputElement>(null);
  const [compressing, setCompressing] = useState(false);
  async function handleChange(e: React.ChangeEvent<HTMLInputElement>) {
    const raw = e.target.files?.[0];
    if (!raw) return;
    if (raw.size > 20 * 1024 * 1024) { alert("File too large (max 20 MB)."); return; }
    setCompressing(true);
    try { onChange(await compressToWebP(raw)); } catch { onChange(raw); }
    finally { setCompressing(false); if (ref.current) ref.current.value = ""; }
  }
  const sizeLabel = file ? (() => { const kb = file.size / 1024; return kb > 1024 ? `${(kb / 1024).toFixed(1)} MB` : `${Math.round(kb)} KB`; })() : null;
  return (
    <div>
      <input ref={ref} type="file" accept="image/*" className="hidden" onChange={handleChange} />
      <button type="button" onClick={() => ref.current?.click()} disabled={compressing}
        className={`w-full rounded-xl border-2 px-3 py-2.5 text-sm font-semibold transition-all flex items-center gap-2 ${compressing ? "border-teal-200 bg-teal-50 text-teal-500 cursor-wait" : file ? "border-emerald-300 bg-emerald-50 text-emerald-700" : "border-dashed border-teal-200 bg-white text-[#0f766e] hover:border-[#0c7b93] hover:bg-teal-50"}`}>
        {compressing ? <><span className="animate-spin">⏳</span><span className="text-xs">Compressing to WebP…</span></>
        : file ? <><span className="text-emerald-500 shrink-0">✓</span><span className="truncate text-xs flex-1">{file.name}</span><span className="text-xs text-emerald-600 shrink-0">{sizeLabel} · Change</span></>
...
function UrgencySection({ lots }: { lots: ParkingLot[] }) {
  ...
  return (
    <div className="rounded-2xl overflow-hidden shadow-sm border-2 border-[#0c7b93]/20" style={{ background: "linear-gradient(135deg,#0c7b93 0%,#064e3b 100%)" }}>
      <div className="px-6 py-7 text-white">
        {pctFull >= 70 && <div className="inline-flex items-center gap-2 rounded-full bg-red-500/20 border border-red-400/40 px-3 py-1 mb-4"><span className="w-2 h-2 rounded-full bg-red-400 animate-pulse inline-block" /><span className="text-xs font-bold text-red-200 uppercase tracking-wide">{pctFull}% of slots taken</span></div>}
        ...
        {totalCapacity > 0 && (
          <div className="rounded-xl bg-black/20 border border-white/10 px-4 py-3">
            ...
            <div className="w-full h-3 rounded-full bg-white/10 overflow-hidden">
              <div className={`h-3 rounded-full transition-all ${pctFull >= 80 ? "bg-red-400" : pctFull >= 50 ? "bg-amber-400" : "bg-emerald-400"}`} style={{ width: `${pctFull}%` }} />
            </div>
...
function BookingModal({ lot, settings, onClose }: { lot: ParkingLot; settings: Settings; onClose: () => void }) {
  ...
  const maxVehicles = getMaxVehicles(lot);

  const [vehicles, setVehicles] = useState<VehicleForm[]>([{ vehicle_type: defaultType, plate_number: "", make_model: "", color: "", or_cr_number: "", driver_id_type: "driver_license", driver_id_number: "", or_cr_file: null, id_photo_file: null }]);
  ...
  const parkingFee = vehicles.reduce((sum, v) => sum + (getRate(lot, v.vehicle_type, settings) ?? 0) * days, 0);
  const total = parkingFee + settings.platformFee + settings.processingFee;

  function addVehicle() {
    if (vehicles.length >= maxVehicles) return;
    setVehicles(v => [...v, { vehicle_type: defaultType, plate_number: "", make_model: "", color: "", or_cr_number: "", driver_id_type: "driver_license", driver_id_number: "", or_cr_file: null, id_photo_file: null }]);
  }
  ...
  function validateVehicles(): string | null {
    const plates = vehicles.map(v => v.plate_number.trim().toUpperCase()).filter(Boolean);
    if (new Set(plates).size !== plates.length)
      return "Duplicate plate number detected. Each vehicle must have a unique plate number.";
    for (const [i, v] of vehicles.entries()) {
      ...
      if (getAvailable(lot, v.vehicle_type) <= 0) return `No available ${v.vehicle_type} slots for this lot.`;
    }
    return null;
  }
  ...
  async function handleGcashFileChange(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    finally { setGcashCompressing(false); if (gcashInputRef.current) gcashInputRef.current.value = ""; }
  }

  function handleNextToPayment() { setError(null); const err = validateVehicles(); if (err) { setError(err); return; } setStep("payment"); }
  function handleNextToConfirm() { setError(null); if (!gcashFile) { setError("Please upload your GCash payment screenshot."); return; } setStep("confirm"); }
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      for (const [i, v] of vehicles.entries()) {
        ...
        fd.append("or_cr", v.or_cr_file); fd.append("id_photo", v.id_photo_file);
        fd.append("vehicle_index", String(i)); fd.append("plate_number", v.plate_number.toUpperCase());
        const r = await fetch("/api/parking/upload-docs", { method: "POST", body: fd });
        const d = await r.json();
        if (!r.ok) { setError(d.error ?? "Photo upload failed."); setStep("vehicles"); return; }
        uploadedPaths.push({ vehicle_index: i, or_cr_path: d.or_cr_path, id_photo_path: d.id_photo_path });
      }
      ...
      if (!gr.ok) { setError(gd.error ?? "GCash upload failed."); setStep("payment"); return; }
      setUploadProgress("Creating your booking…");
      const res = await fetch("/api/parking/reserve", {
        method: "POST", headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          lot_id: lot.id,
          vehicles: vehicles.map((v, i) => ({ vehicle_type: v.vehicle_type, plate_number: v.plate_number.toUpperCase(), make_model: v.make_model || null, color: v.color || null, or_cr_number: v.or_cr_number, driver_id_type: v.driver_id_type, driver_id_number: v.driver_id_number, or_cr_path: uploadedPaths[i]?.or_cr_path ?? null, id_photo_path: uploadedPaths[i]?.id_photo_path ?? null })),
          park_date_start: dateStart, total_days: days,
          gcash_proof_path: gd.path, gcash_transaction_reference: gcashRef.trim() || null,
        }),
      });
      const data = await res.json();
      if (!res.ok) { setError(data.error ?? "Booking failed."); setStep("confirm"); return; }
      setReference(data.reference); setStep("success");
    } catch { setError("Network error. Please try again."); setStep("confirm"); }
  }
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center" style={{ backgroundColor: "rgba(0,0,0,0.65)" }}
      ...
      <div className="w-full sm:max-w-lg bg-white rounded-t-3xl sm:rounded-2xl overflow-hidden shadow-2xl max-h-[95vh] flex flex-col">
        ...
        {/* Header */}
        <div className="px-5 py-4 border-b border-teal-100 flex items-center justify-between shrink-0" style={{ background: "linear-gradient(135deg,#064e3b,#0c7b93)" }}>
          <div>
            <p className="text-xs text-white/60 font-bold uppercase tracking-wide">
              {step === "vehicles" ? "Step 1 of 3 — Vehicle Details" : step === "payment" ? "Step 2 of 3 — Payment" : step === "confirm" ? "Step 3 of 3 — Review" : step === "submitting" ? "Submitting…" : "Booking Submitted"}
            </p>
        ...
        {step === "success" && (
          <div className="flex-1 overflow-y-auto p-6 text-center">
            ...
            <div className="mt-5 flex flex-col gap-2">
              <Link href="/dashboard/parking" onClick={onClose} className="w-full rounded-xl bg-[#0c7b93] px-4 py-3 text-sm font-bold text-white hover:bg-[#085f72] transition-colors">View My Parking Bookings →</Link>
              <button onClick={onClose} className="w-full rounded-xl border-2 border-teal-200 px-4 py-2.5 text-sm font-semibold text-[#134e4a] hover:bg-teal-50">Close</button>
            </div>
          </div>
        )}
        ...
        {step === "confirm" && (
          <div className="flex-1 overflow-y-auto p-5 space-y-4">
            ...
            <div className="rounded-xl border-2 border-teal-100 overflow-hidden">
              <div className="bg-teal-50 px-4 py-2 text-xs font-bold text-[#134e4a] uppercase">{vehicles.length} Vehicle{vehicles.length > 1 ? "s" : ""}</div>
              {vehicles.map((v, i) => (
                <div key={i} className={`px-4 py-3 border-b border-teal-50 last:border-0 text-sm ${i % 2 === 0 ? "bg-white" : "bg-teal-50/40"}`}>
                  <div className="flex items-center gap-2">
                    <div className={`w-1 h-4 rounded-full shrink-0 ${TYPE_ACCENT[v.vehicle_type] ?? "bg-gray-300"}`} />
                    <span className={`text-xs font-bold px-2 py-0.5 rounded-full ${TYPE_BADGE[v.vehicle_type] ?? ""}`}>{getAcceptedTypes(lot).find(t => t.value === v.vehicle_type)?.emoji} {v.vehicle_type}</span>
                    <span className="font-semibold text-[#134e4a] font-mono">{v.plate_number}</span>
                  </div>
            ...
            <div className="rounded-xl bg-emerald-50 border border-emerald-200 px-4 py-3 text-sm">
              <div className="flex items-center gap-2"><span className="text-emerald-600">✓</span><span className="text-emerald-800 font-semibold">GCash screenshot uploaded</span></div>
            ...
            <div className="rounded-xl border-2 border-teal-200 overflow-hidden">
              ...
              <div className="px-4 py-3 space-y-2 text-sm">
                <div className="flex justify-between"><span className="text-gray-600">Parking fee ({vehicleCount} vehicle{vehicleCount > 1 ? "s" : ""} × {days} day{days > 1 ? "s" : ""})</span><span className="font-semibold">{peso(parkingFee)}</span></div>
                <div className="flex justify-between text-gray-500"><span>Platform Service Fee</span><span>{peso(settings.platformFee)}</span></div>
                <div className="flex justify-between text-gray-500"><span>Payment Processing Fee</span><span>{peso(settings.processingFee)}</span></div>
            ...
            <div className="flex gap-3">
              ...
              <button onClick={handleSubmit} className="flex-1 rounded-xl bg-[#0c7b93] px-4 py-3 text-sm font-bold text-white hover:bg-[#085f72] transition-colors">Submit Booking →</button>
            </div>
          </div>
        )}
        ...
        {step === "payment" && (
          <div className="flex-1 overflow-y-auto p-5 space-y-5">
            <div><h3 className="font-bold text-[#134e4a] mb-1">Step 2 — Pay via GCash</h3><p className="text-xs text-[#0f766e]">Send the exact amount below, then upload your screenshot.</p></div>
            <div className="rounded-xl bg-teal-50 border-2 border-teal-200 overflow-hidden">
              <div className="px-4 py-3 space-y-1.5 text-sm">
                <div className="flex justify-between"><span className="text-gray-600">Parking fee ({vehicleCount} vehicle{vehicleCount > 1 ? "s" : ""} × {days} day{days > 1 ? "s" : ""})</span><span className="font-semibold">{peso(parkingFee)}</span></div>
                <div className="flex justify-between text-xs text-gray-400"><span>Platform Service Fee</span><span>{peso(settings.platformFee)}</span></div>
                <div className="flex justify-between text-xs text-gray-400"><span>Payment Processing Fee</span><span>{peso(settings.processingFee)}</span></div>
              </div>
              <div className="flex justify-between font-black text-base px-4 py-3 border-t-2 border-teal-200 bg-teal-100/40"><span className="text-[#134e4a]">Total to Send</span><span className="text-[#0c7b93]">{peso(total)}</span></div>
            </div>
            ...
            <div>
              <label className={labelCls}>GCash Payment Screenshot <span className="text-red-500">*</span></label>
              ...
              <button type="button" onClick={() => gcashInputRef.current?.click()} disabled={gcashCompressing}
                className={`w-full rounded-xl border-2 px-3 py-3 text-sm font-semibold transition-all flex items-center gap-2 ${gcashCompressing ? "border-teal-200 bg-teal-50 text-teal-500 cursor-wait" : gcashFile ? "border-emerald-300 bg-emerald-50 text-emerald-700" : "border-dashed border-teal-200 bg-white text-[#0f766e] hover:border-[#0c7b93] hover:bg-teal-50"}`}>
                {gcashCompressing ? <><span className="animate-spin">⏳</span><span className="text-xs">Compressing…</span></> : gcashFile ? <><span className="text-emerald-500 shrink-0">✓</span><span className="truncate text-xs flex-1">{gcashFile.name}</span><span className="text-xs text-emerald-600 shrink-0">Change</span></> : <><span>📸</span><span className="text-xs flex-1">Upload GCash screenshot</span><span className="ml-auto text-xs text-teal-400 shrink-0">Required</span></>}
              </button>
            </div>
            <div>
            ...
            {error && <div className="rounded-xl bg-red-50 border border-red-200 px-4 py-3 text-sm text-red-700">{error}</div>}
            <div className="flex gap-3">
              <button onClick={() => setStep("vehicles")} className="flex-1 rounded-xl border-2 border-teal-200 px-4 py-3 text-sm font-semibold text-[#134e4a] hover:bg-teal-50">← Back</button>
              <button onClick={handleNextToConfirm} className="flex-1 rounded-xl bg-[#0c7b93] px-4 py-3 text-sm font-bold text-white hover:bg-[#085f72] transition-colors">Review Booking →</button>
            </div>
          </div>
        )}
          ...
          <div className="flex-1 overflow-y-auto p-5 space-y-5">
            <div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div><label className={labelCls}>Start Date</label><input type="date" value={dateStart} min={today} onChange={e => setDateStart(e.target.value)} className={inputCls} /></div>
                <div><label className={labelCls}>Days (max {settings.maxDays})</label><input type="number" min={1} max={settings.maxDays} value={days} onChange={e => setDays(Math.max(1, Math.min(settings.maxDays, parseInt(e.target.value) || 1)))} className={inputCls} /></div>
              </div>
            </div>
            ...
            <div>
              <div className="flex items-center justify-between mb-3">
                ...
                <div className="flex items-center gap-2">
                  <span className="text-xs text-gray-400">{vehicles.length}/{maxVehicles} max</span>
                  {vehicles.length < maxVehicles && <button onClick={addVehicle} type="button" className="text-xs font-bold text-[#0c7b93] hover:underline">+ Add vehicle</button>}
                </div>
              </div>

              <div className="space-y-4">
                {vehicles.map((v, i) => (
                  <div key={i} className={`rounded-xl border-2 p-4 relative ${CARD_BG[i % 2]}`}>
                    {/* Vehicle header with accent bar */}
                    <div className="flex items-center gap-2 mb-3">
                      <div className={`w-1 h-5 rounded-full shrink-0 ${TYPE_ACCENT[v.vehicle_type] ?? "bg-gray-300"}`} />
                      <p className="text-xs font-bold text-[#0c7b93]">Vehicle {i + 1}</p>
                      {v.plate_number && (
                        <span className="font-mono text-xs font-bold text-[#134e4a] bg-white/80 px-2 py-0.5 rounded-full border border-teal-200">{v.plate_number}</span>
                      )}
                      <span className={`text-xs font-bold px-2 py-0.5 rounded-full ${TYPE_BADGE[v.vehicle_type] ?? ""}`}>
                        {getAcceptedTypes(lot).find(t => t.value === v.vehicle_type)?.emoji} {v.vehicle_type}
                      </span>
                      {vehicles.length > 1 && <button onClick={() => removeVehicle(i)} type="button" className="ml-auto text-red-400 hover:text-red-600 text-xs font-semibold">Remove</button>}
                    </div>
                    ...
                    <div className="mb-3">
                      ...
                      <div className={`grid gap-2 grid-cols-${acceptedTypes.length}`}>
                        {acceptedTypes.map(t => {
                          ...
                          return (
                            <button key={t.value} type="button" onClick={() => updateVehicle(i, "vehicle_type", t.value)} disabled={avail <= 0}
                              className={`rounded-xl border-2 px-3 py-2.5 text-left transition-all ${v.vehicle_type === t.value ? "border-[#0c7b93] bg-white shadow-sm" : avail <= 0 ? "border-gray-100 bg-gray-50 opacity-50 cursor-not-allowed" : "border-teal-100 bg-white/60 hover:border-teal-300"}`}>
                              ...
                              <div className={`text-xs font-bold mt-0.5 ${avail === 0 ? "text-red-500" : avail <= 3 ? "text-amber-600" : "text-emerald-600"}`}>{avail === 0 ? "Full" : `${avail} left`}</div>
                            </button>
                          );
                        })}
                      </div>
                    ...
                    <div className="grid grid-cols-2 gap-2 mb-3">
                      <div className="col-span-2"><label className={labelCls}>Plate Number *</label><input type="text" value={v.plate_number} onChange={e => updateVehicle(i, "plate_number", e.target.value.toUpperCase())} placeholder="ABC 1234" className={inputCls} style={{ textTransform: "uppercase" }} /></div>
                      <div><label className={labelCls}>Make &amp; Model</label><input type="text" value={v.make_model} onChange={e => updateVehicle(i, "make_model", e.target.value)} placeholder="Toyota Vios" className={inputCls} /></div>
                      <div><label className={labelCls}>Color</label><input type="text" value={v.color} onChange={e => updateVehicle(i, "color", e.target.value)} placeholder="White" className={inputCls} /></div>
                      <div><label className={labelCls}>OR/CR Number *</label><input type="text" value={v.or_cr_number} onChange={e => updateVehicle(i, "or_cr_number", e.target.value)} placeholder="Registration #" className={inputCls} /></div>
                      <div><label className={labelCls}>ID Type *</label><select value={v.driver_id_type} onChange={e => updateVehicle(i, "driver_id_type", e.target.value)} className={inputCls}>{ID_TYPES.map(t => <option key={t.value} value={t.value}>{t.label}</option>)}</select></div>
                      <div className="col-span-2"><label className={labelCls}>ID Number *</label><input type="text" value={v.driver_id_number} onChange={e => updateVehicle(i, "driver_id_number", e.target.value)} placeholder="ID number" className={inputCls} /></div>
                    </div>

                    <div className="space-y-2">
                      <p className="text-xs font-bold text-[#134e4a]">📎 Document Photos <span className="text-red-500">*</span></p>
                      <p className="text-xs text-gray-400">Auto-compressed to WebP for fast upload.</p>
                      <FileUploadButton label="Upload OR/CR photo (front)" file={v.or_cr_file} onChange={f => updateVehicle(i, "or_cr_file", f)} />
                      <FileUploadButton label={`Upload ${ID_TYPES.find(t => t.value === v.driver_id_type)?.label ?? "ID"} photo`} file={v.id_photo_file} onChange={f => updateVehicle(i, "id_photo_file", f)} />
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div className="rounded-xl bg-teal-50 border-2 border-teal-200 overflow-hidden">
              <div className="px-4 py-3 space-y-1.5 text-sm">
                <div className="flex justify-between"><span className="text-[#0f766e]">Parking fee ({vehicleCount} vehicle{vehicleCount > 1 ? "s" : ""} × {days} day{days > 1 ? "s" : ""})</span><span className="font-semibold text-[#134e4a]">{peso(parkingFee)}</span></div>
                <div className="flex justify-between text-xs text-gray-400"><span>Platform Service Fee</span><span>{peso(settings.platformFee)}</span></div>
                <div className="flex justify-between text-xs text-gray-400"><span>Payment Processing Fee</span><span>{peso(settings.processingFee)}</span></div>
              </div>
              <div className="flex justify-between font-black text-base px-4 py-3 border-t-2 border-teal-200 bg-teal-100/40"><span className="text-[#134e4a]">Total</span><span className="text-[#0c7b93]">{peso(total)}</span></div>
            </div>

            {error && <div className="rounded-xl bg-red-50 border border-red-200 px-4 py-3 text-sm text-red-700">{error}</div>}
            <button onClick={handleNextToPayment} type="button" className="w-full min-h-[52px] rounded-xl bg-[#0c7b93] font-bold text-white text-sm hover:bg-[#085f72] transition-colors shadow-sm">Next — Upload Payment →</button>
          </div>
        )}
      </div>
    </div>
    ...
    <div className={`rounded-2xl border-2 bg-white shadow-sm overflow-hidden transition-all hover:shadow-md ${isFull ? "border-red-200" : "border-teal-200 hover:border-[#0c7b93]"}`}>
      ...
      <div className="p-5">
        ...
        <div className="flex flex-wrap gap-2 mb-4">
          {lot.accepts_car        && <span className="rounded-full bg-emerald-50 border border-emerald-200 px-2 py-0.5 text-xs font-semibold text-emerald-800">🚗 {peso(lot.car_rate_cents ?? settings.carRate)}/day</span>}
          {lot.accepts_motorcycle && <span className="rounded-full bg-amber-50 border border-amber-200 px-2 py-0.5 text-xs font-semibold text-amber-800">🏍️ {peso(lot.motorcycle_rate_cents ?? settings.motorcycleRate)}/day</span>}
          {lot.accepts_van && lot.van_rate_cents && <span className="rounded-full bg-blue-50 border border-blue-200 px-2 py-0.5 text-xs font-semibold text-blue-800">🚐 {peso(lot.van_rate_cents)}/day</span>}
        </div>
        <button onClick={onBook} disabled={isFull} className={`w-full min-h-[44px] rounded-xl font-bold text-sm transition-colors ${isFull ? "bg-gray-100 text-gray-400 cursor-not-allowed" : "bg-[#0c7b93] text-white hover:bg-[#085f72] shadow-sm"}`}>
  ...
  return (
    <div className="min-h-screen" style={{ backgroundColor: "#f8f6f0" }}>
      <div className="relative overflow-hidden" style={{ background: "linear-gradient(135deg,#064e3b 0%,#0c7b93 55%,#0891b2 100%)" }}>
        <svg className="absolute bottom-0 left-0 w-full pointer-events-none" viewBox="0 0 1440 80" preserveAspectRatio="none" style={{ opacity: 0.12 }}><path d="M0,40 C360,80 720,0 1080,40 C1260,60 1380,20 1440,40 L1440,80 L0,80 Z" fill="white"/></svg>
      ...
      <div className="mx-auto max-w-5xl px-4 py-10 sm:px-6 lg:px-8 space-y-10">
        ...
        <div>
          ...
          {lots.length === 0
            ? <div className="rounded-2xl border-2 border-teal-100 bg-white p-10 text-center"><div className="text-4xl mb-3">🚗</div><p className="font-bold text-[#134e4a]">Parking lots coming soon</p></div>
            : <div className="grid gap-4 sm:grid-cols-2">{lots.map(lot => <LotCard key={lot.id} lot={lot} settings={settings} onBook={() => setSelectedLot(lot)} />)}</div>}
        </div>
        <div>
        ...
        <div className="rounded-2xl border-2 border-amber-200 bg-amber-50 p-6">
          ...
          <div className="grid sm:grid-cols-2 gap-3">
            {[{ icon: "🪪", label: "Valid Government-Issued ID", sub: "UMID, Driver's License, Passport, PhilSys, etc." }, { icon: "📄", label: "OR/CR", sub: "Official Receipt & Certificate of Registration." }].map(r => (
              <div key={r.label} className="flex items-start gap-3 rounded-xl bg-white/70 p-3 border border-amber-200">
                <span className="text-2xl shrink-0">{r.icon}</span>
                <div><p className="text-sm font-semibold text-amber-900">{r.label}</p><p className="text-xs text-amber-700 mt-0.5">{r.sub}</p></div>
              </div>
            ))}
          </div>
        </div>
