...
export async function GET() {
  ...
  const { data } = await supabase.from("parking_settings").select("*").eq("id", 1).maybeSingle();
...
export async function POST(request: NextRequest) {
  ...
  const { error } = await supabase.from("parking_settings").upsert({
    ...
    default_truck_rate_cents:      body.default_truck_rate_cents != null ? Number(body.default_truck_rate_cents) : null,
