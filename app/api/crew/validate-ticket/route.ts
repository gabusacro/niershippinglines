...
export async function GET(request: NextRequest) {
  ...
  if (needsId) {
    ...
    // This handles the case where the passenger has verified their ID on a
    // previous booking and is reusing it for this one.
    if (!idVerification && booking.user_id) {
      ...
      if (profileIds && profileIds.length > 0) {
        // Among reusable IDs, prefer verified and name-matching first
    ...
    // Covers guest checkouts or bookings where user_id is null.
  ...
  // ── Compute age from birthdate for senior citizen cross-check ─────────────
  // Useful for crew to visually confirm 60+ age even if ID shows it
